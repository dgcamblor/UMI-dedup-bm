...
dbm_barplot <- function(summarized_data, stat, level_order, full_names, title, image_path, hjust = -0.55, vjust = 0.5) {
  if (length(level_order) != length(summarized_data[["pipeline_names"]])) {
  ...
  summarized_data$pipeline_names <- factor(summarized_data$pipeline_names, levels = level_order, labels = full_names) 
  
  tiff(image_path, units = "px", width = 2828, height = 2044, res = 400)
  plot <- ggplot(summarized_data, aes(x = pipeline_names, y = !!sym(aes_y), fill = pipeline_names)) +
